...
export function handleRouteError(error: unknown, context?: string): never {
  ...
  if (err?.status === 401) {
    throw new Response(`${message}${ERROR_MAP[401].description}`, { status: 401 });
  }
  if (err?.status === 403) {
    throw new Response(`${message}${ERROR_MAP[403].description}`, { status: 403 });
  }
  if (err?.status === 404) {
    throw new Response(`${message}${ERROR_MAP[404].description}`, { status: 404 });
  }
  ...
  const isOffline = !globalThis.navigator?.onLine
    || err instanceof TypeError
    || err?.message === "Failed to fetch"
    || err?.code === "ECONNREFUSED";

  if (isOffline) {
    throw new Response(`${message}${ERROR_MAP[503].description}`, { status: 503 });
  }

  throw new Response(`${message}${ERROR_MAP[500].description}`, { status: 500 });
}

export const getErrorComponent = (status?: number): ComponentType<ErrorComponentProps> => {
