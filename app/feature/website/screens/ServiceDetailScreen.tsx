...
export function ServiceDetailScreen({ serviceId, token, onBack }: ServiceDetailScreenProps) {
  const { data: service, isLoading, error } = useService(serviceId, token || undefined);
    ...
    return (
      <div className="min-h-screen">
        <EmptyState
          ...
          description="The requested service could not be found or may have been removed."
          className="mt-8"
          actions={[
            {
                ...
                <ul className="space-y-2">
                  {service.services.map((serviceItem, index) => (
                    <li key={index} className="flex items-start gap-3">
                      ...
                      <span className="text-gray-700">{typeof serviceItem === 'string' ? serviceItem : JSON.stringify(serviceItem)}</span>
                    </li>
                  ))}
                </ul>
              </div>
