...
type ColorVariant = 'yellow' | 'blue' | 'green' | 'red' | 'gray' | 'outline' | 'handdrawn' | 'handdrawn-yellow' | 'handdrawn-blue';
...
export const IconContainer = ({
    ...
    const isHandDrawn = ['handdrawn', 'handdrawn-yellow', 'handdrawn-blue'].includes(color);
    ...
    return (
        <div className={cn('flex-shrink-0', className)}>
            ...
            >
                ...
                {number && <span className={colorVariants[color as keyof typeof colorVariants]?.text}>{number}</span>}
                {text && <span className={colorVariants[color as keyof typeof colorVariants]?.text}>{text}</span>}
            </div>
        </div>
    );
};