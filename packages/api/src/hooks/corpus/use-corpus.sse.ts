// packages/api/src/hooks/corpus/use-corpus.sse.ts
import { useCallback } from "react";
import { corpusApi } from "../../clients/corpus.api";
import { corpusKeys } from "./use-corpus";
import {
  getUploadKey,
  useUploadManager,
...
export function useUploadCorpusEntry() {
  ...
  const upload = useCallback(
    async (files: File[]): Promise<boolean> => {
      ...
      for (const file of files) {
        ...
        try {
          await corpusApi.uploadCorpusEntry(
            file,
            { title: file.name },
            (event) => {
