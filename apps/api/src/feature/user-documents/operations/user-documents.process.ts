...
export async function processUserDocument(
  ...
  const chunks = chunkDocument(
    ...
    {
      chunkSize: UserDocumentConfig.CHUNK_SIZE,
      overlapSize: UserDocumentConfig.CHUNK_OVERLAP,
    },
  );
  ...
  const batchSize = UserDocumentConfig.CHUNK_SIZE;
  ...
  logger.info(
    ...
    "User document processed successfully",
  );
...
/**
 * Check if a user document exists for a session
 * @param sessionId - The chat session ID
 * @returns Document status with metadata
 */
export async function getUserDocumentStatus(
  sessionId: string,
...
/**
 * Delete a user document collection
 * @param sessionId - The chat session ID
 * @returns True if deleted, false if collection didn't exist
 */
export async function deleteUserDocument(sessionId: string): Promise<boolean> {
  const collectionName = getUserDocumentCollectionName(sessionId);
...
/**
 * Check if a session has an associated user document collection
 * @param sessionId - The chat session ID
