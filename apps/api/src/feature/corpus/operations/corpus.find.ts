...
import { documentsTable, bookmarksTable } from "../corpus.schema";
...
export const findCorpusEntry = async <K extends CorpusColumnKey>(
  field: K,
...
export async function findCorpusEntries(
  query: CorpusFilters,
): Promise<DbManyResult<Corpus>> {
  const filters = [];
  ...
  const result = await paginate<"documentsTable", Corpus>(
    "documentsTable",
    documentsTable,
    {
      ...query,
...
export async function getCorpusShareInfo({
  documentId,
}: ShareCorpusParams): Promise<CorpusShareInfo> {
  ...
  const shareableLink = `https://mahakama.app/corpus/${documentId}`;

  const shareInfo: CorpusShareInfo = {
    documentId,
