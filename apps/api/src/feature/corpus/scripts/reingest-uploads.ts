...
import { updateCorpusEntry } from "../operations/corpus.update";
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (existing) {
        ...
        await updateCorpusEntry("id", existing.id, {
          version: nextVersion,
          lastUpdated: today(),
        });
        ...
        const created = await createCorpusEntry({
        ...
        if (!created.ok || !created.data) {
          throw new Error("Failed to create corpus entry row");
        }
        documentId = created.data.id;
        logger.info(
          { filename: file, documentId },
          "Created corpus entry row for uploaded file",
        );
      }
