...
import { DocumentSource } from "../documents.types";
import type {
  RAGChunk,
  RAGSource,
  RAGContext,
 ...
 * @returns RAG context from document
 */
export async function retrieveDocumentContext(
  sessionId: string,
  query: string,
  topK: number = DocumentConfig.QUERY_TOP_K_WITH_USER_DOC,
): Promise<RAGContext> {
...
/**
 * Merge document context with legal corpus context
 *
...
/**
 * Check if a session has an uploaded document
 *
 * @param sessionId - The chat session ID
