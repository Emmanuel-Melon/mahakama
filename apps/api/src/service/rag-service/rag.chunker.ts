...
import { CHUNK_CONFIG } from "../embedding-service/embeddings.config";

export const splitIntoSections = (text: string): Section[] => {
  const matches = [...text.matchAll(CHUNK_CONFIG.SECTION_HEADER)];
...
export const chunkDocument = (
  ...
  const overlapSize = Math.min(
    options.overlapSize ?? CHUNK_CONFIG.DEFAULT_OVERLAP_SIZE,
  ...
  const firstHeaderIndex = text.search(CHUNK_CONFIG.SECTION_HEADER_FIRST);
...
const findWordBoundary = (text: string, start: number, end: number): number => {
  const tolerance = Math.max(
    Math.floor((end - start) * CHUNK_CONFIG.WORD_BOUNDARY_TOLERANCE),
    1,
  );
  ...
  for (let i = end; i >= lowerBound; i--) {
    if (CHUNK_CONFIG.WORD_BOUNDARY.test(text[i] ?? "")) {
