...
actually built — `buildChunkId` uses the chunk's *logical* `chunk.id` plus a
...
to collide with something, the success path *overwrites* the correct
...
The schema *has* a `jurisdiction` column, and it's returned in metadata, but
...
time against *all* rows missing an embedding — chunks that legitimately
...
attempts (good) but *also* excluded from the housekeeping delete, which only
...
nothing to backfill). If the primary returns *some but not all* of the
